...
class MoviesAdapter(private val layoutInflater: LayoutInflater, private val imageLoader: ImageLoader, private val animationLoader: AnimationLoader) :
    RecyclerView.Adapter<MoviesAdapter.MovieViewHolder>() {

    interface OnItemClickListener {
        fun onItemClick(movie: MovieViewModel)
        fun onFavourite(movie: MovieViewModel)
        fun onUnfavoured(movieId: Int)
    }
    ...
    fun addData(movieList: List<MovieViewModel>) {
    ...
    class MovieViewHolder(
        ...
        fun addValuesOnHolder(movie: MovieViewModel, position: Int) = movie.run {
            ...
            view.favouriteButton.setOnCheckedChangeListener { compoundButton, isChecked ->
                if (isChecked) {
                    onItemClickListener?.onFavourite(this)
                } else {
                    onItemClickListener?.onUnfavoured(this.id)
                }
            }
            ...
            view.favouriteButton.isChecked = movie.isFavourite
